...
import { Commands, Strings, Engine } from '../../../src'

import { Section } from 'dodi'

savor
  .add(
    'should make sure it expects required args',
    (context: Context, done: Completion) => {
  ...
  .add(
    'should make sure it does not run without a name',
    (context: Context, done: Completion) => {
      const cmd = new Commands.Init()
      ...
      savor.promiseShouldFail(Engine.run(cmd), done, (error) => {
        ...
        context
          ...
          .to.equal(Strings.ArgumentIsMissingString('name'))
      })
    }
  )
  .add(
    'should make sure it does not run without a template',
    (context: Context, done: Completion) => {
      const cmd = new Commands.Init()
      process.env.CARMEL_USER_HOME = context.dir
      ...
      savor.promiseShouldFail(
        ...
        (error) => {
          ...
          context
            .expect(error.message)
            .to.equal(Strings.ArgumentIsMissingString('template'))
        }
      )
    }
  )
  ...
  // .add(
  //   'should skip creating if a product exists already',
  //   (context: Context, done: Completion) => {
  //     const cmd = new Commands.Init()
  ...
  //         context
  //           .expect(error.message)
  //           .to.equal(Strings.ProductAlreadyExistsString())
  //       }
  //     )
  //   }
  // )
