...
export default class EarlyAccessScreen extends Screen {
  ...
  addToCart(cart) {
    ...
    newCart.items[Carmel1K.item.id] = Object.assign({}, Carmel1K.item, { quantity: 1 })

    Data.Cache.cacheItem("__shoppingCart", newCart)
              .then(() => this.triggerRedirect("/checkout"))
  }
  ...
  renderForm() {
    ...
    return <Card style={{ margin: "10px", width }}>
        ...
        <List twoLine style={{ padding, borderTop: "1px solid #eeeeee", borderBottom: "1px solid #eeeeee" }}>
        {
          Carmel1K.benefits.map(benefit => <SimpleListItem graphic="check_circle" text={benefit.title} />)
        }
        </List>
        <CardActions style={{ justifyContent: "center", paddingTop: "40px"}}>
          <Button
            ...
            <ButtonIcon icon="credit_card" style={{ marginLeft: "5px", fontSize: "24px" }}/>
            <Typography
              use="headline5"
              tag="div"
              style={{ textAlign: "center", color: "#ffffff", padding: "0px 20px 0px 20px" }}>
              €{Carmel1K.item.price}
            </Typography>
            <ButtonIcon icon="arrow_forward" style={{ marginLeft: "5px", fontSize: "24px" }}/>
          </Button>
         </CardActions>
         <CardActions style={{ justifyContent: "center", paddingBottom: "40px" }}>
           <Typography
             use="caption"
             tag="div"
             style={{
