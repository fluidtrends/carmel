...
namespace carmel {
  ...
  void token::distribute(name from, name to, asset quantity, string memo)
  {
      eosio_assert(to == _self, "wrong contract");
      eosio_assert(quantity.symbol.is_valid(), "invalid quantity");
      eosio_assert(quantity.amount >= 5, "at least 5 EOS required");
...
extern "C" void apply(uint64_t receiver, uint64_t code, uint64_t action)
{
    if (code == "eosio.token"_n.value && action == "transfer"_n.value)
    {
        eosio::execute_action(
            eosio::name(receiver), eosio::name(code), &carmel::token::distribute
        );
    }
    else if (code == receiver)
    {
        switch (action)
        {
            EOSIO_DISPATCH_HELPER(carmel::token, (create)(issue)(transfer)(open)(close)(retire))
        }
    }
}
